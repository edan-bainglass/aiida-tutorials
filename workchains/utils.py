...
def birch_murnaghan(V, E0, V0, B0, B01):
    ...
    r = (V0 / V)**(2.0 / 3.0)
    return E0 + 9.0 / 16.0 * B0 * V0 * (r - 1.0)**2 * (2.0 + (B01 - 4.0) *
                                                       (r - 1.0))
...
def plot_eos(eos_pk):
    ...
    if "result" in eos_calc.outputs:
        result = eos_calc.outputs.result.dict.eos
    else:
        result = eos_calc.outputs.eos.dict.eos

    data = []
    for V, E, units in result:
        data.append((V, E))
